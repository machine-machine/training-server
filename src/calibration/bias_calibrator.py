...
class NumericValidator:
    ...
    def validate_array(arr: np.ndarray, name: str = "array",
                       min_length: int = 1, allow_nan: bool = False) -> np.ndarray:
...
class BiasCoefficients:
    ...
    def from_dict(cls, data: Dict[str, Any]) -> "BiasCoefficients":
        ...
        if "slippage" in data:
            ...
            coeffs.slippage_volatility_coef = s.get("volatility_coef", coeffs.slippage_volatility_coef)
            coeffs.slippage_liquidity_coef = s.get("liquidity_coef", coeffs.slippage_liquidity_coef)

        if "latency" in data:
            l = data["latency"]
            coeffs.latency_base_mean_ms = l.get("base_mean_ms", coeffs.latency_base_mean_ms)
            coeffs.latency_base_std_ms = l.get("base_std_ms", coeffs.latency_base_std_ms)
            coeffs.latency_congestion_multiplier_low = l.get("congestion_multiplier_low", coeffs.latency_congestion_multiplier_low)
            coeffs.latency_congestion_multiplier_high = l.get("congestion_multiplier_high", coeffs.latency_congestion_multiplier_high)

        if "mev" in data:
            m = data["mev"]
            coeffs.mev_sandwich_probability = m.get("sandwich_probability", coeffs.mev_sandwich_probability)
            coeffs.mev_sandwich_cost_bps = m.get("sandwich_cost_bps", coeffs.mev_sandwich_cost_bps)
            coeffs.mev_jito_discount_factor = m.get("jito_discount_factor", coeffs.mev_jito_discount_factor)
        ...
        if "inclusion" in data:
            ...
            coeffs.inclusion_congestion_decay = i.get("congestion_decay", coeffs.inclusion_congestion_decay)
            coeffs.inclusion_partial_fill_threshold = i.get("partial_fill_threshold", coeffs.inclusion_partial_fill_threshold)

        if "staleness" in data:
            st = data["staleness"]
            coeffs.max_quote_age_ms = st.get("max_quote_age_ms", coeffs.max_quote_age_ms)
            coeffs.quote_staleness_penalty_bps = st.get("penalty_bps", coeffs.quote_staleness_penalty_bps)
...
class BiasCalibrator:
    ...
    def __init__(self, db: Optional["DatabaseManager"] = None):
        ...
        self.last_report: Optional[CalibrationReport] = None
    ...
    async def calibrate(
        ...
        if observations is None or len(observations) < min_observations:
            ...
            report.warnings.append(
                f"Insufficient observations: {len(observations) if observations else 0} < {min_observations}"
            )
    ...
    def _fit_slippage_model(self, observations: List[BiasObservation], report: CalibrationReport):
        ...
        def slippage_model(params, X):
            ...
            relative_size, volatility, liquidity = X[:, 0], X[:, 1], X[:, 2]

            size_slip = base * np.exp(size_exp * relative_size)
    ...
    def _fit_latency_model(self, observations: List[BiasObservation], report: CalibrationReport):
        ...
        if self.coefficients.latency_base_mean_ms > 0:
            ...
            logger.warning("latency_base_mean_ms is zero or negative, skipping congestion multiplier calibration")
        ...
        predicted_mean = self.coefficients.latency_base_mean_ms * (
            ...
            (self.coefficients.latency_congestion_multiplier_high -
             self.coefficients.latency_congestion_multiplier_low) * congestions
        )
    ...
    def _fit_inclusion_model(self, observations: List[BiasObservation], report: CalibrationReport):
        ...
        partial_fills = [o for o in observations if 0 < o.partial_fill_fraction < 1.0]
        if len(partial_fills) >= 5:
            # Find threshold where partial fills become common
            fill_fractions = [o.trade_size_sol / (o.pool_liquidity_usd / 200) for o in partial_fills]
    ...
    def _fit_mev_model(self, observations: List[BiasObservation], report: CalibrationReport):
        ...
        valid_obs = [
            o for o in observations
            if o.included and o.realized_slippage_bps is not None
        ]
        ...
        non_mev_obs = [o for o in valid_obs if not o.mev_detected]
        ...
        if len(jito_mev) >= 3 and len(non_jito_mev) >= 3:
            ...
            jito_cost = np.median([o.realized_slippage_bps - o.quoted_slippage_bps for o in jito_mev])
            non_jito_cost = np.median([o.realized_slippage_bps - o.quoted_slippage_bps for o in non_jito_mev])
    ...
    def _walk_forward_validation(
        ...
        for train_idx, test_idx in tscv.split(indices):
            ...
            valid_train = [o for o in train_obs if o.included and o.realized_slippage_bps is not None]
            ...
            if len(valid_train) >= 10:
                ...
                # Evaluate on test
                valid_test = [o for o in test_obs if o.included and o.realized_slippage_bps is not None]

                if len(valid_test) >= 5:
                    test_slippages = [o.realized_slippage_bps - o.quoted_slippage_bps for o in valid_test]
    ...
    def load_state(self, state: Dict[str, Any]):
        ...
        logger.info(f"Loaded bias calibrator state with {len(self.calibration_history)} history entries")
    ...
    def to_hotpath_params(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "latency_congestion_multiplier_low": self.coefficients.latency_congestion_multiplier_low,
            "latency_congestion_multiplier_high": self.coefficients.latency_congestion_multiplier_high,
