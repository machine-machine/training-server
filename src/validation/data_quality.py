...
class DataQualityValidator:
    ...
    LEAKAGE_PATTERNS = [
        "future", "next_", "will_", "outcome", "result",
        "target", "label", "y_", "profit", "pnl_future",
    ]
        ...
        for field in self.required_fields:
            if field not in df.columns:
                ...
                if null_count > 0:
                    ...
                    severity = ValidationSeverity.WARNING if null_pct < 10 else ValidationSeverity.ERROR
                    issues.append(ValidationIssue(
                        code="NULL_VALUES",
                        message=f"Field '{field}' has {null_count} null values ({null_pct:.1f}%)",
                        severity=severity,
                        field=field,
                        count=null_count,
                    ))
        ...
        for field, spec in self.field_specs.items():
            ...
            if spec.get("min") is not None:
                ...
                if violations > 0:
                    issues.append(ValidationIssue(
                        code="BELOW_MIN",
                        message=f"Field '{field}' has {violations} values below minimum ({spec['min']})",
                        severity=ValidationSeverity.WARNING,
                        field=field,
                        count=violations,
                    ))
            ...
            if spec.get("max") is not None:
                ...
                if violations > 0:
                    issues.append(ValidationIssue(
                        code="ABOVE_MAX",
                        message=f"Field '{field}' has {violations} values above maximum ({spec['max']})",
                        severity=ValidationSeverity.WARNING,
                        field=field,
                        count=violations,
                    ))
            ...
            if spec.get("values"):
                ...
                if invalid.sum() > 0:
                    issues.append(ValidationIssue(
                        code="INVALID_VALUE",
                        message=f"Field '{field}' has {invalid.sum()} invalid values",
                        severity=ValidationSeverity.WARNING,
                        field=field,
                        count=invalid.sum(),
    ...
    def _detect_outliers(self, df: pd.DataFrame) -> Tuple[List[ValidationIssue], Dict[str, int]]:
        ...
        for col in numeric_cols:
            ...
            if outlier_count > 0:
                ...
                severity = ValidationSeverity.INFO if outlier_pct < 1 else ValidationSeverity.WARNING
                issues.append(ValidationIssue(
                    code="OUTLIERS",
    ...
    def validate_train_test_split(
        ...
        if timestamp_col in train_df.columns and timestamp_col in test_df.columns:
            ...
            if train_max >= test_min:
                overlap_count = (
                    (train_df[timestamp_col] >= test_min).sum() +
                    (test_df[timestamp_col] <= train_max).sum()
                )
                issues.append(ValidationIssue(
