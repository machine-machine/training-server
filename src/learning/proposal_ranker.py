...
class BanditArm:
    ...
    def update(self, reward: float):
        ...
        self.variance = (
            (self.variance * (self.pull_count - 1) + delta * delta2) /
            self.pull_count
        )
...
class ProposalRanker:
    ...
    def rank_candidates(
        ...
        deep_scorer: Optional[Callable[[Dict[str, float]], Tuple[float, float]]] = None,
        n_top: int = 10,
        regime: Optional[str] = None,
    ) -> RankingResult:
        ...
        exploration_count = sum(
            1 for c in top_candidates[:n_top]
            if c.source in exploration_sources
        )
