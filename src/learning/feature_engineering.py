...
class FeatureSet:
    ...
    LEGACY_FEATURE_NAMES: List[str] = field(default_factory=lambda: [
        "liquidity_usd",
        "volume_24h_usd",
        "vol_liq_ratio",
        "holder_count",
...
class FeatureEngineer:
    ...
    def extract(self, raw: Dict[str, Any]) -> FeatureSet:
        ...
        features[FeatureIndex.LARGE_HOLDER_CHURN] = np.tanh(
            raw.get("large_holder_churn", 0) / 10.0
        )

        features[FeatureIndex.MINT_AUTHORITY_REVOKED] = (
            1.0 if raw.get("mint_authority_revoked", not raw.get("mint_authority_enabled", False)) else -1.0
        )
        ...
        features[FeatureIndex.BUY_VOLUME_RATIO] = (
            raw.get("buy_volume_ratio", 0.5) * 2.0 - 1.0
        )
        ...
        features[FeatureIndex.VWAP_DEVIATION] = np.tanh(
            raw.get("vwap_deviation", 0) / 10.0
        )
        ...
        # === ON-CHAIN RISK (33-42) ===
        features[FeatureIndex.CONTRACT_IS_MINTABLE] = (
            -1.0 if raw.get("contract_is_mintable", raw.get("mint_authority_enabled", False)) else 1.0
        )
        ...
        features[FeatureIndex.BENFORD_LAW_PVALUE] = (
            raw.get("benford_law_pvalue", 0.5) * 2.0 - 1.0
        )
        ...
        features[FeatureIndex.RUG_PULL_ML_SCORE] = (
            1.0 - 2.0 * raw.get("rug_pull_ml_score", raw.get("goplus_risk_score", 0))
        )
    ...
    def extract_for_signal_classification(
