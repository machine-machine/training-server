...
- Optimized batch sizes for 48GB unified memory
"""
...
# Check for PyTorch availability with smart detection
TORCH_AVAILABLE = False
TORCH_DEVICE: str | None = None


def _detect_torch_availability() -> tuple[bool, str | None]:
    """Detect PyTorch availability with smart device detection.

    Priority: CUDA > MPS (Apple Silicon) > CPU.
    On macOS we now auto-enable MPS/CPU when available to avoid silent degraded mode.
    """
    try:
        ...
        logger.warning("PyTorch not available (import failed); LSTM will use fallback")
        return False, None

    if os.getenv("COLDPATH_DISABLE_TORCH", "0") == "1":
        logger.info("PyTorch explicitly disabled via COLDPATH_DISABLE_TORCH=1")
        return False, None

    device = None

    # Prefer CUDA when available (rare on Apple Silicon but keep for dev hosts)
    try:
        if torch.cuda.is_available():
            device = "cuda"
            gpu_name = torch.cuda.get_device_name(0)
            logger.info(f"PyTorch available with CUDA: {gpu_name}")
    except Exception as e:  # pragma: no cover
        logger.debug(f"CUDA probe failed: {e}")

    # Apple Silicon: auto-enable MPS when supported
    if device is None and hasattr(torch.backends, "mps"):
        try:
            if torch.backends.mps.is_built():
                if torch.backends.mps.is_available():
                    device = "mps"
                    logger.info("PyTorch available with MPS (Apple Silicon)")
                else:
                    logger.warning(
                        "PyTorch MPS backend built but not available "
                        "(update macOS or enable hardware acceleration)"
                    )
            else:
                logger.debug("PyTorch built without MPS support")
        except Exception as e:  # pragma: no cover
            logger.debug(f"MPS probe failed: {e}")

    # CPU fallback (now allowed on macOS without extra env flags)
    if device is None:
        device = "cpu"
        logger.info("PyTorch available on CPU backend")

    return True, device


TORCH_AVAILABLE, TORCH_DEVICE = _detect_torch_availability()
...
class LSTMPricePredictor:
    ...
    def predict(self, sequence: np.ndarray, current_price: float) -> PricePrediction:
        ...
        if sequence.shape[1] != self.sequence_length:
            ...
            if sequence.shape[1] < self.sequence_length:
                padding = np.zeros(
                    (1, self.sequence_length - sequence.shape[1], self.input_features)
                )
    ...
    def predict_batch(
        ...
        """
        predictions = []
        for seq, price in zip(sequences, current_prices, strict=False):
            pred = self.predict(seq, price)
            predictions.append(pred)
        return predictions

    def cleanup(self) -> None:
        """Release GPU resources by moving model to CPU and clearing GPU cache.

        Handles both CUDA and MPS (Apple Silicon) devices.
        """
        if not TORCH_AVAILABLE:
            return
        ...
        if hasattr(self, "device") and self.device and str(self.device) != "cpu":
            ...
            if str(self.device) == "cuda":
                torch.cuda.empty_cache()
            # MPS doesn't have explicit cache clearing, but we can sync
...
def prepare_sequences(
    ...
    for i in range(n_samples):
        ...
        features[:, FEATURE_MOMENTUM] = np.concatenate(
            [[0] * 5, (seq_prices[5:] - seq_prices[:-5]) / seq_prices[:-5]]
        )

        # Order flow (volume-weighted price change)
        features[:, FEATURE_ORDER_FLOW] = np.concatenate(
            [[0], returns * (seq_volumes[1:] / (seq_volumes[:-1] + 1e-8))]
        )
