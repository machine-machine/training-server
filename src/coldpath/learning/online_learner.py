...
class OnlineLearningConfig:
    ...
    adaptive_lr_decay: float = 0.99  # Decay factor per update
    adaptive_lr_min: float = 0.001  # Minimum learning rate
    gradient_norm_threshold: float = 1.0  # Gradient clipping threshold
    ...
    regime_adjustments: dict[str, float] = field(
        default_factory=lambda: {
            ...
            "chop": 0.8,  # Slightly conservative in chop
            "mev_heavy": 0.5,  # Very conservative with high MEV
        }
    )
...
class OnlineLearningState:
    ...
    coefficient_momentum: np.ndarray | None = None
    ...
    gradient_history: deque[float] = field(default_factory=lambda: deque(maxlen=100))

    # Regime tracking
    current_regime: str = "unknown"
    regime_update_counts: dict[str, int] = field(default_factory=dict)
    ...
    def to_dict(self) -> dict[str, Any]:
        ...
        return {
            ...
            "current_coefficients": self.current_coefficients.tolist()
            ...
            "regime_update_counts": self.regime_update_counts,
        }
...
class OnlineLearner:
    ...
    def __init__(
        ...
        self.state.current_learning_rate = self.config.learning_rate
    ...
    async def update_if_ready(self) -> dict[str, Any] | None:
    ...
    def _perform_update(self) -> dict[str, Any]:
        ...
        # Track regimes in batch
        regime_counts: dict[str, int] = {}
        for o in batch:
            regime = o.regime
            regime_counts[regime] = regime_counts.get(regime, 0) + 1
            self.state.regime_update_counts[regime] = (
                self.state.regime_update_counts.get(regime, 0) + 1
            )

        # Determine dominant regime for this update
        if regime_counts:
            dominant_regime = max(regime_counts.items(), key=lambda x: x[1])[0]
            self.state.current_regime = dominant_regime
        ...
        if self.config.use_adaptive_lr and gradient_norm > self.config.gradient_norm_threshold:
            gradient = gradient * (self.config.gradient_norm_threshold / gradient_norm)
        ...
        if self.config.use_adaptive_lr:
            lr = self.state.current_learning_rate * self.config.adaptive_lr_decay
            lr = max(lr, self.config.adaptive_lr_min)
            self.state.current_learning_rate = lr

        # Regime-aware adjustment
        regime_mult = self.config.regime_adjustments.get(self.state.current_regime, 1.0)
        lr *= regime_mult

        # Apply momentum
        self.state.coefficient_momentum = (
            self.config.momentum * self.state.coefficient_momentum - lr * gradient
        )
        ...
        logger.info(
            ...
            f"lr={lr:.6f}, regime={self.state.current_regime}"
        )
        ...
        return {
            ...
            "coefficient_change": float(np.linalg.norm(update)),
            "learning_rate": lr,
    ...
    def predict(self, features: np.ndarray) -> float:
        ...
        if self.config.use_feature_scaling:
            features_scaled = (features - self.state.feature_means) / (
                self.state.feature_stds + 1e-8
            )
        else:
    ...
    def cleanup_old_confirmations(self, max_age_hours: float = 24.0) -> int:
        ...
        to_remove = [
            tid
            for tid, outcome in self._pending_confirmations.items()
