...
class AdvancedRegimeDetector:
    ...
    def __init__(
        ...
        self._last_features: RegimeFeatures8D | None = None
    ...
    def fit(self, observations: np.ndarray) -> "AdvancedRegimeDetector":
        ...
        if len(observations) < self.min_samples:
            logger.warning(f"Insufficient samples: {len(observations)} < {self.min_samples}")
            self._is_fitted = False
            return self
    ...
    def get_current_state(
        self,
        features: RegimeFeatures8D | None = None,
    ) -> AdvancedRegimeState:
        ...
        features = features or self._last_features or RegimeFeatures8D(0, 0, 0, 0, 1, 0, 0, 1)
    ...
    def get_regime_weights(
        self,
        regime: AdvancedRegime | None = None,
    ) -> RegimeModelWeights:
    ...
    def export_state(self) -> dict[str, Any]:
        ...
        export = {
            ...
            "regime_weights": {k: v.to_dict() for k, v in self._regime_weights.items()},
        }
        ...
        if self._is_fitted and HMM_AVAILABLE and hasattr(self._model, "means_"):
            export["hmm_params"] = {
                ...
                "covars": (
                    self._model.covars_.tolist() if hasattr(self._model, "covars_") else None
                ),
                "transmat": self._model.transmat_.tolist(),
                "startprob": self._model.startprob_.tolist(),
            }
            export["state_mapping"] = {str(k): v.value for k, v in self._state_to_regime.items()}
    ...
    def load_state(self, state: dict[str, Any]):
        ...
        self._current_regime = AdvancedRegime(state.get("current_regime", "normal"))
        ...
        if "state_mapping" in state:
            self._state_to_regime = {
                int(k): AdvancedRegime(v) for k, v in state["state_mapping"].items()
            }
...
def compute_8d_features(
    ...
    for i in range(window, n):
        ...
        # Sandwich rate
        features[i, 6] = np.mean(sandwich_counts[i - window : i]) / (
            np.mean(sandwich_counts) + 1e-10
        )
