...
class FeatureScore:
    ...
    vif: float | None = None  # Variance Inflation Factor
    selected: bool = False
...
class MutualInformationCalculator:
    ...
    def __init__(self, config: MIConfig | None = None):
...
class MutualInformationSelector:
    ...
    def select_features(
        ...
        feature_scores = self.calculator.compute_mi_scores(X, y, feature_names, is_classification)
        ...
        logger.info(
            f"Selected {len(selected_indices)}/{n_features} features, total MI: {total_mi:.4f}"
        )
    ...
    def _select_mrmr(
        ...
        remaining = {i for i in remaining if scores[i].mi_score >= self.config.mi_threshold}
    ...
    def get_regime_features(self, regime: str) -> list[int] | None:
    ...
    def get_feature_recommendations(
        ...
        for score in scores:
            if score.mi_score < self.config.mi_threshold:
                ...
                investigate.append(f"{score.feature_name} (high VIF: {score.vif:.2f})")
