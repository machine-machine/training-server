...
class IsolationForestRugDetector:
    ...
    def __init__(
        ...
        self._feature_depths: np.ndarray | None = None

    def fit(self, X: np.ndarray, y: np.ndarray | None = None) -> "IsolationForestRugDetector":
    ...
    def predict_risk_batch(self, X: np.ndarray) -> list[RugRiskResult]:
        ...
        results = []
        for i, (raw_score, label) in enumerate(zip(raw_scores, labels, strict=False)):
            risk_score = self._normalize_score(raw_score)
            risk_level = self._get_risk_level(risk_score)
    ...
    def _compute_feature_importance(self, X_scaled: np.ndarray) -> None:
        ...
        for idx in sample_indices:
            ...
            self.model.decision_function(sample)
    ...
    def _get_top_risk_features(
        self, x_scaled: np.ndarray, top_k: int = 5
    ...
    def get_feature_importance(self) -> dict[str, float]:
        ...
        return dict(zip(self.feature_names, normalized, strict=False))
