...
class EnhancedTokenFeatures(TokenFeatures):
    ...
    lp_unlock_timestamp: int | None = None  # When LP unlocks (None = unlocked)
...
class DeployerTracker:
    ...
    def record_success(
        self, token_mint: str, lifespan_seconds: float, volume_sol: float, max_mcap_sol: float
    ):
    ...
    def get_profile(self, deployer_wallet: str) -> DeployerProfile | None:
        """Get deployer profile if exists."""
        return self._profiles.get(deployer_wallet)

    def get_deployer_for_token(self, token_mint: str) -> str | None:
    ...
    def _update_avg_mcap(self, profile: DeployerProfile, new_mcap: float):
        ...
        if n <= 1:
            ...
            profile.avg_max_mcap_sol = (profile.avg_max_mcap_sol * (n - 1) + new_mcap) / n
...
class BuySellAnalyzer:
    ...
    def analyze(self, token_mint: str, current_liquidity_sol: float) -> dict[str, Any]:
        ...
        large_sells = sum(
            1 for t in trades if not t["is_buy"] and t["amount_sol"] > large_threshold
        )
...
class MutualInfoResult:
    ...
    feature_score_std: dict[str, float] | None = None  # Optional score std dev
    ...
    def summary(self) -> str:
        ...
        lines.append(
            f"  Selected: {len(self.selected_features)}/{len(self.feature_scores)} features"
        )
...
class SyntheticDataResult:
    ...
    def summary(self) -> str:
        ...
        lines.append(
            f"  Original: {self.original_samples} samples (ratio: {self.original_class_ratio:.2%})"
        )
...
class TrainingResult:
    ...
    mi_result: MutualInfoResult | None = None  # MI feature selection results
    synthetic_result: SyntheticDataResult | None = None  # Synthetic data augmentation results
...
class FraudModel:
    ...
    def __init__(
        ...
        synthetic_method: SyntheticMethod | str = SyntheticMethod.SMOTE,
        synthetic_ratio: float = 1.0,
        adversarial_ratio: float = 0.1,
        # Advanced MI selector options
        use_advanced_mi: bool = True,
        mi_method: str | None = "mrmr",  # "univariate_mi", "mrmr", "jmi", "cmim"
        ...
        self.advanced_mi_selector: MutualInformationSelector | None = None
        self.advanced_mi_result: SelectionResult | None = None
        ...
        self.synthetic_result: SyntheticDataResult | None = None
        ...
        self.temporal_generator: TemporalSyntheticGenerator | None = None
        ...
        self.feature_mask: np.ndarray | None = None  # Boolean mask for selected features
        self.selected_feature_names: list[str] | None = None
        self.mi_scores: dict[str, float] | None = None
        self.mi_score_std: dict[str, float] | None = None
        self.mi_result: MutualInfoResult | None = None
    ...
    def _compute_mi_scores(self, X: np.ndarray, y: np.ndarray, n_neighbors: int) -> np.ndarray:
    ...
    def _compute_mutual_information(self, X: np.ndarray, y: np.ndarray) -> MutualInfoResult:
        ...
        if n_samples < 10 or minority_count < 2:
            logger.warning("Insufficient samples for stable MI; keeping all features")
            ...
            if runs == 1:
                ...
                for _ in range(runs):
                    X_sample, y_sample = self._stratified_bootstrap(X, y, self.mi_bootstrap_frac)
        ...
        self.mi_scores = dict(zip(self.feature_names, mi_scores, strict=False))
        self.mi_score_std = (
            dict(zip(self.feature_names, mi_std, strict=False)) if mi_std is not None else None
        )

        # Determine which features to select
        # Sort by MI score descending
        ranked = sorted(
            zip(self.feature_names, mi_scores, range(len(self.feature_names)), strict=False),
            key=lambda x: -x[1],
        )
    ...
    def _apply_smote(self, X: np.ndarray, y: np.ndarray) -> tuple[np.ndarray, np.ndarray]:
        ...
        # Guard: SMOTE requires at least 2 minority samples
        minority_count = int(np.sum(y == 1))
        if minority_count < 2:
            logger.warning(f"SMOTE skipped: insufficient minority samples ({minority_count} < 2)")
            return X, y
        ...
        try:
            ...
            logger.info(
                f"SMOTE: {len(y)} -> {len(y_resampled)} samples ({self.synthetic_method.value})"
            )
    ...
    def train(
        ...
        if self.use_mi_selection:
            ...
            logger.info(f"Feature selection: {X_scaled.shape[1]} -> {X_selected.shape[1]} features")
        else:
            X_selected = X_scaled
    ...
    def _try_tpot(self, X: np.ndarray, y: np.ndarray) -> TrainingResult | None:
    ...
    def _try_autosklearn(self, X: np.ndarray, y: np.ndarray) -> TrainingResult | None:
    ...
    def _train_random_forest(self, X: np.ndarray, y: np.ndarray) -> TrainingResult:
        """Train with RandomForest (guaranteed to work)."""
        logger.info("Training with RandomForest classifier")
    ...
    def analyze_synthetic_data_impact(
        ...
        tokens: list[TokenFeatures],
        labels: list[int],
        methods: list[SyntheticMethod] | None = None,
        cv_folds: int = 5,
    ) -> dict[str, Any]:
        ...
        for method in methods:
            ...
            try:
                ...
                if method != SyntheticMethod.NONE:
                    X_aug, y_aug, synth_result = self._augment_with_synthetic_data(X_selected, y)
                else:
                ...
                f1_scores = cross_val_score(model, X_aug, y_aug, cv=cv_folds, scoring=f1_scorer)
