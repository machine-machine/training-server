...
class DriftState:
    ...
    reference_feature_stats: dict[str, tuple[float, float]] | None = None
...
class DriftDetector:
    ...
    def __init__(
        ...
        on_drift_alert: Callable[[DriftAlert], None] | None = None,
    ):
    ...
    def _check_staleness(self) -> DriftAlert | None:
        ...
        if age >= self.config.max_parameter_age_hours:
            ...
            return DriftAlert(
                ...
                timestamp_ms=now_ms,
                description=(
                    f"Parameters are {age:.1f} hours old "
                    f"(max: {self.config.max_parameter_age_hours}h)"
                ),
    ...
    def _check_accuracy(self) -> DriftAlert | None:
        ...
        if degradation >= self.config.accuracy_degradation_threshold:
            ...
            return DriftAlert(
                ...
                timestamp_ms=now_ms,
                description=(
                    f"Accuracy dropped from {self.state.baseline_accuracy:.1%} "
    ...
    def _check_bias(self) -> DriftAlert | None:
        ...
        biases = [p.predicted_probability - (1.0 if p.actual_class else 0.0) for p in recent]
    ...
    def _check_regime_mismatch(self) -> DriftAlert | None:
        """Check for regime prediction mismatch."""
        # Get predictions with current regime info
        with_regime = [
            p for p in self._predictions if p.current_regime is not None and p.current_regime != ""
        ]
        ...
        mismatches = sum(1 for p in recent if p.regime_at_prediction != p.current_regime)
    ...
    def _check_feature_drift(self) -> DriftAlert | None:
        ...
        for i in range(n_features):
            ...
            if (
                ks_stat >= self.config.feature_drift_threshold
                and p_value < self.config.feature_drift_p_value
            ):
        ...
        if len(drifted_features) > 0:
            ...
            return DriftAlert(
                ...
                timestamp_ms=now_ms,
                description=(
                    f"Feature drift detected in {len(drifted_features)}/{n_features} features"
                ),
                details={
