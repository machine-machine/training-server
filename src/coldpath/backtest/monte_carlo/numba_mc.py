...
def run_monte_carlo_simulation(
    ...
    # Sharpe statistics
    sharpe_stats = compute_sharpe_confidence_interval(final_equities, cfg.initial_capital)
    ...
    if np.any(valid_mask):
        result.avg_win_rate = float(np.mean(win_counts[valid_mask] / trade_counts[valid_mask]))
    ...
    # Probability of ruin
    prob_ruin, _ = compute_probability_of_ruin(max_drawdowns, cfg.ruin_threshold_pct)
...
class MonteCarloEngine:
    ...
    def run(
        ...
        return run_monte_carlo_simulation(prices, entry_signals, self.config, signal_config)
