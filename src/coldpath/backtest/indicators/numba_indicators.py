...
try:
    from numba import float64, int64, njit, prange  # noqa: F401
...
if NUMBA_AVAILABLE:
    ...
    def _macd_batch(
        ...
        for i in prange(n_tokens):
            ml, sl, hist = _macd_core(prices[i, :], fast_period, slow_period, signal_period)
            macd_lines[i, :] = ml
            signal_lines[i, :] = sl
            histograms[i, :] = hist
    ...
    def _bollinger_batch(
        ...
        for i in prange(n_tokens):
            u, m, lb, p = _bollinger_core(prices[i, :], period, num_std)
            upper[i, :] = u
            middle[i, :] = m
            lower[i, :] = lb
            pct_b[i, :] = p
    ...
    def _macd_batch(prices, fast_period, slow_period, signal_period):
        n_tokens = prices.shape[0]
        results = [
            _macd_core(prices[i], fast_period, slow_period, signal_period) for i in range(n_tokens)
        ]
        return (
    ...
    def _realized_volatility(returns, period, annualization_factor):
        ...
        result[: period - 1] = np.nan
        for i in range(period - 1, n):
            result[i] = np.sqrt(np.mean(returns[i - period + 1 : i + 1] ** 2)) * np.sqrt(
...
def compute_macd(
    ...
    """
    macd_line, signal_line, histogram = _macd_core(prices, fast_period, slow_period, signal_period)
    return MACDResult(
        macd_line=macd_line,
...
def compute_combined_signals(
    ...
    macd_lines, signal_lines, histograms = _macd_batch(prices, macd_fast, macd_slow, macd_signal)
