...
class PerpBacktestEngine:
    ...
    def run_backtest(
        ...
        for idx, row in price_data.iterrows():
            ...
            self._check_liquidations(
                market,
                low
                if market in self.positions and self.positions[market].side == PositionSide.LONG
                else high,
            )
            ...
            market_signals = signals[
                (signals["timestamp"] == self.current_time) & (signals["market"] == market)
            ]
    ...
    def _process_signal(self, signal: pd.Series, price: float):
        ...
        if sig == 0:
            ...
            # Long
            self._open_position(market, PositionSide.LONG, size, price, leverage, tp, sl)
        elif sig == -1:
            # Short
            self._open_position(market, PositionSide.SHORT, size, price, leverage, tp, sl)
    ...
    def _open_position(
        ...
        if margin > self.capital * self.config.max_position_size_pct:
            logger.warning(
                f"Position size exceeds max: {margin} > "
                f"{self.capital * self.config.max_position_size_pct}"
            )
            return
    ...
    def _check_liquidations(self, market: str, extreme_price: float):
        ...
        if position.side == PositionSide.LONG:
            if extreme_price <= position.liquidation_price:
                logger.info(
                    f"LIQUIDATION: {market} {position.side.value} @ {position.liquidation_price}"
                )
    ...
    def _process_funding(
        ...
        funding_row = funding_data[
            (funding_data["market"] == market) & (funding_data["timestamp"] <= self.current_time)
        ].tail(1)
    ...
    def _update_equity(self, price_data: pd.DataFrame, idx: int):
        ...
        for _market, position in self.positions.items():
            # Get current price
    ...
    def _calculate_sharpe(self, risk_free_rate: float = 0.02) -> float | None:
