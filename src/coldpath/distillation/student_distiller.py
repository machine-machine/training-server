...
The student model uses 7 "legacy-style" feature names that map to positions
in the 50-feature vector defined in `feature_engineering.py`:

| Student Feature    | 50-Feature Index | Notes                           |
|--------------------|------------------|---------------------------------|
| liquidity_usd      | POOL_TVL_SOL (0) | Direct mapping                  |
| fdv_usd            | TOTAL_SUPPLY (12)| Proxy - uses total_supply       |
| volume_24h         | VOLUME_ACCEL (26)| Proxy - uses volume_acceleration|
| holder_count       | HOLDER_COUNT (15)| Direct mapping                  |
| top_holder_pct     | TOP_10_HOLD (14) | Proxy - uses concentration      |
| fraud_score        | RUG_PULL_ML (42) | Direct mapping (inverted)       |
| token_age_hours    | POOL_AGE_SEC (1) | Direct mapping (÷3600 for hrs)  |

CRITICAL: The feature names in HOTPATH_FEATURES are for the STUDENT model only.
The actual 50-feature vector names are in FeatureSet.FEATURE_NAMES.
"""
...
class StudentConfig:
    ...
    default_transforms: dict[str, str] = field(
...
class StudentDistiller:
    ...
    def distill(self, teacher: TeacherResult) -> StudentResult | None:
        ...
        X_student, unit_conversion_applied = self._extract_student_features(teacher.features)
        ...
        # 3. Split into train/validation using chronological holdout.
        # Trading data is temporal; random splits leak future structure into training.
        n_samples = len(X_transformed)
        val_size = max(1, int(round(n_samples * self.config.validation_split)))
        if n_samples - val_size < 10:
            logger.error(
                "Insufficient samples for chronological validation split: total=%s val=%s",
                n_samples,
                val_size,
            )
            return None
        ...
        calibration_error = self._expected_calibration_error(student_probs, hard_val, n_bins=10)
        ...
        if correlation < self.config.min_correlation:
            logger.warning(
                f"Student rejected: correlation {correlation:.3f} < {self.config.min_correlation}"
            )
            return None
    ...
    def _extract_student_features(
        ...
        for i, feat_name in enumerate(HOTPATH_FEATURES):
            ...
            if source_idx >= full_features.shape[1]:
                logger.warning(f"Feature index {source_idx} out of bounds for {feat_name}")
                return None, False
    ...
    def _compute_transforms(self, X: np.ndarray) -> tuple[list[FeatureTransformSpec], np.ndarray]:
    ...
    def _platt_scale(self, probs: np.ndarray, labels: np.ndarray) -> PlattCalibration:
        """Fit Platt scaling: P(y=1|f) = 1 / (1 + exp(A*f + B))."""
        from sklearn.linear_model import LogisticRegression

        # Check if we have at least 2 classes in the labels
        unique_labels = np.unique(labels)
        if len(unique_labels) < 2:
            logger.warning(
                f"Platt scaling skipped: only {len(unique_labels)} class(es) in validation set. "
                f"Using default calibration."
            )
            # Return default calibration (identity transform)
