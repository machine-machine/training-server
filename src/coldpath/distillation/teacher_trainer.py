...
class TeacherConfig:
    ...
    outcome_type: str | None = None  # None = all types
...
class TeacherTrainer:
    ...
    def __init__(self, db: DatabaseManager, config: TeacherConfig | None = None):
        self.db = db
        self.config = config or TeacherConfig()

    async def train(self) -> TeacherResult | None:
        ...
        if len(outcomes) < self.config.min_outcomes:
            logger.warning(
                f"Insufficient training outcomes before merge: {len(outcomes)} < "
                f"{self.config.min_outcomes}; attempting scan/hotpath supplement"
            )
        ...
        hotpath_trade_outcomes = await self.db.get_hotpath_trade_outcomes(
            since_hours=self.config.lookback_hours,
            limit=10000,
        )
        ...
        all_outcomes = self._merge_hotpath_trade_outcomes(
            all_outcomes,
            hotpath_trade_outcomes,
        )

        if len(all_outcomes) < self.config.min_outcomes:
            logger.warning(
                f"Insufficient merged outcomes: {len(all_outcomes)} < {self.config.min_outcomes}"
            )
            return None
    ...
    def _merge_hotpath_trade_outcomes(
        self,
        merged: list[dict[str, Any]],
        hotpath_trade_outcomes: list[dict[str, Any]],
    ) -> list[dict[str, Any]]:
        """Supplement merged outcomes with persisted HotPath trade telemetry."""
        seen = {(outcome.get("mint", ""), outcome.get("timestamp_ms", 0)) for outcome in merged}

        for outcome in hotpath_trade_outcomes:
            normalized = self._normalize_training_outcome(outcome)
            if not normalized:
                continue
            key = (normalized.get("mint", ""), normalized.get("timestamp_ms", 0))
            if key in seen:
                continue
            seen.add(key)
            merged.append(normalized)
    ...
    def _normalize_training_outcome(self, outcome: dict[str, Any]) -> dict[str, Any] | None:
