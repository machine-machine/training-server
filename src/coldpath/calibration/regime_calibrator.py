...
class CalibratedParameters:
    ...
    transition_type: RegimeTransitionType | None = None
    samples_used: int = 0
...
class RegimeTriggeredCalibrator:
    ...
    def __init__(
        self,
        config: RegimeCalibrationConfig | None = None,
        on_calibration: Callable[[CalibratedParameters], None] | None = None,
    ):
        ...
        # Transition state
        self._transition_start_ms: int | None = None
        self._transition_from_params: CalibratedParameters | None = None
        self._transition_to_params: CalibratedParameters | None = None
        self._active_transition: RegimeTransition | None = None
    ...
    async def _check_and_calibrate(self, metrics: MarketMetrics) -> CalibratedParameters | None:
        ...
        if transition.transition_type != RegimeTransitionType.NONE:
            ...
            if (
                now_ms - self._last_calibration_ms
                < self.config.min_time_between_calibrations_seconds * 1000
            ):
                return None
    ...
    def _fast_calibrate(
        ...
        alpha = (
            ...
            else self.config.ema_alpha_slow
        )
        ...
        if transition.transition_type in [
            RegimeTransitionType.CONGESTION_SURGE,
            RegimeTransitionType.MEV_SURGE,
        ]:
        ...
        return CalibratedParameters(
            ...
            survivorship_discount_adjustment=self._get_survivorship_adjustment(
    ...
    def get_active_transition(self) -> RegimeTransition | None:
    ...
    def get_current_params(self) -> CalibratedParameters:
        ...
        return CalibratedParameters(
            ...
            slippage_multiplier=(
                (1 - blend) * from_p.slippage_multiplier + blend * to_p.slippage_multiplier
            ),
            inclusion_multiplier=(
                (1 - blend) * from_p.inclusion_multiplier + blend * to_p.inclusion_multiplier
            ),
            mev_exposure_multiplier=(
                (1 - blend) * from_p.mev_exposure_multiplier + blend * to_p.mev_exposure_multiplier
            ),
            latency_multiplier=(
                (1 - blend) * from_p.latency_multiplier + blend * to_p.latency_multiplier
            ),
            survivorship_discount_adjustment=to_p.survivorship_discount_adjustment,
            confidence_multiplier=(
                (1 - blend) * from_p.confidence_multiplier + blend * to_p.confidence_multiplier
            ),
